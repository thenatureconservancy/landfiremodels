...
**[LANDFIRE BpS models](https://www.landfire.gov/bps-models.php){target='blank'}** are **reference condition models** that define natural vegetation communities and their historical disturbance regimes. On the **[modify](./modifyModel.html)** page you will learn more about these models and how to **modify** them to represent current landscapes and simulate possible future conditions.
</div>
<br>

### SyncroSim software

LANDFIRE currently uses SyncroSim and its ST-Sim package as the modeling platform to develop and deliver its STSMs. SyncroSim is available for free and is the latest in a series of related STSM software packages including Path and the [Vegetation Dynamics Development Tool](https://essa.com/explore-essa/tools/vddt/){target='blank'} (VDDT). SyncroSim is a flexible modeling platform that can be used to simulate virtually any ecosystem and provides user control over much of its functionality. The model is stochastic (randomly determined) and can be run either spatially or non-spatially. We recommend that users read the [overview of SyncroSim](http://docs.syncrosim.com/getting_started/overview.html){target='blank'} and follow the steps below for downloading Syncrosim and St-Sim.
...
#### Step 2: Get required packages:
To work through this tutorial you will need to load 2 packages: 1) St-Sim and 2) landfirevegmodels.  There are (or soon will be) 2 options for loading these packages.  The landfirevegmodels package is under development so is in a provisional site, and will be integrated into the SyncroSim based package install pane by the end of September, 2020.

**Option 1** Download package files (currently the only option for lfvegmodels as of September 2020):

+ ST-Sim is located [here](https://syncrosim.com/packages/){target='blank'}. Download into your Downloads folder.
+ The landfirevegmodels package is located [here](https://github.com/ApexRMS/landfirevegmodels/releases){target='blank'}.  You will download the 'landfirevegmodels-1-0-2.ssimpkg' file.  Download into your Downloads folder.

Once downloaded you will load the packages in SyncroSim by clicking File -> Packages... -> Install from File (top right corner of Packages pane), then navigating to your Downloads folder where the two .ssim files should be. 

<br>

**Option 2** Install from Packages pane in SyncroSim:

*As of September 2020 this option is available for the ST-Sim package, but not the lfvegmodels package.*


In the SyncroSim shell, navigate to 'File' -> 'Packages' -> select 'ST-Sim state-and-transition model' (or after September, 2020 'lfvegmodels') -> 'Install'
<center>
